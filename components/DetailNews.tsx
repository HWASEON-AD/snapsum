...
export default function DetailNews({ analysis }: { analysis: NewsAnalysis }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      {analysis.key_points.map((pt, i) => (
        <div key={i} style={{ display: 'flex', gap: 9, background: '#0d0d1a', borderRadius: 11, padding: '9px 11px', border: '1px solid #141422' }}>
          <div style={{ width: 18, height: 18, borderRadius: 5, background: 'rgba(96,165,250,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 800, color: '#60a5fa', flexShrink: 0 }}>
            {i + 1}
          </div>
